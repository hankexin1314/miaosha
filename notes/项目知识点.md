...
- 向登录URL发送请求，后端controller处理请求，跳转到登录页面
- 在登录页面输入账号密码，点击登录后，账号密码通过post请求，传递到服务器（这里密码进行了一次**md5加密**）
...
- 每次访问商品列表页面，就会进行一次数据库查询，查询商品库存，商品信息

- 然后通过model将查询结果返回给前端，前端使用Thymeleaf渲染页面

### 2. 优化

- 查询库存信息，然后再后端手动渲染（WebContext对象，还有Thymeleaf解析器），渲染出一个html页面
- 将html页面存入缓存中，下次再访问首先从缓存中去寻找，如果缓存中有，就避免了查数据库和渲染的过程
- 缓存设置好过期时间，定时刷新
...
- controller收到请求，查询数据库，得到商品的信息，通过model传回给前端，跳转到详情页面，前端Thymeleaf根据查询到的信息渲染页面

### 2. 优化

- 在商品列表点击商品详情信息，直接跳转到商品详情页`/goods_detail.htm?goodsId='+${goods.id}`（一个**静态页面**）
- 详情页向后端`/goods/detail/{goodsId}`发送ajax请求，后端controller查询数据库将信息返回给前端
- 前端使用这些信息渲染静态页面
...
- 首先打开商品详情页（商品id在url中），后端查询数据库，得到秒杀时间，根据秒杀时间判断现在是否处于秒杀时段
...
### 4. 二次优化——隐藏秒杀接口，验证码，自定义注解

#### 4.1 存在的问题

- html是明文，可能会有人利用脚本直接刷秒杀页面，来恶意秒杀
- 代码有些冗余，例如redis过期时间，验证是否登陆都需要自己写逻辑

#### 4.2 避免脚本刷秒杀页面——隐藏秒杀接口，验证码

##### 隐藏秒杀接口

- 之前是点击秒杀按钮，直接访问`/miaosha/domiaosha`，利用post携带goodsId，利用cookie携带userId
